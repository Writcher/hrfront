...
import { deleteProyectoDTO, editProyectoDTO, insertProyectoDTO } from "@/lib/dtos/proyecto";
...
export async function fetchProyectosABM() {
    try {
        const token = await getToken();

        const proyectosRaw = await fetch(`${CONFIG.URL_BASE}${CONFIG.URL_PROYECTOS}?accion=abm`, {
            method: "GET",
            headers: {
                Authorization: `Bearer ${token}`,
            }
