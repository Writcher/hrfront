      ...
      value && (
        <div key={key} className="flex items-center gap-2 border-2 border-gray-700 pl-2 pr-1 py-1 rounded">
          <span className="text-gray-800 text-xs md:text-sm">
            ...
            {key === "filtroTipoAusencia" && `${getNombreTipoAusenciaPorId(value)}`}
            {key === "filtroMes" && `${getNombreMesPorId(value)}`}
            {key === "filtroQuincena" && `${value === 1 ? "Primera Quincena" : "Segunda Quincena"}`}
          </span>
          <IconButton
